...
ap.add_argument(
    "intersection_size", help="The desired size of the intersection", type=int
)
ap.add_argument(
    "label_byte_count",
    nargs="?",
...
while len(recv_set) < min(int_sz, recv_sz):
    ...
    kv_pair = random.choice(sender_list)
    ground_truth_set.add(kv_pair)
    item = kv_pair[0]
    recv_set.add(item)
...
with open("ground_truth.csv", "w") as ground_truth_file:
    for item, label in list(ground_truth_set):
        ground_truth_file.write(item + (("," + label) if label_bc != 0 else "") + "\n")
print("Wrote ground_truth set")