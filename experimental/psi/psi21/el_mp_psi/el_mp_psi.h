...
class NmpParty {
 public:
  ...
  NmpParty(const Options& options);
 ...
 private:
  ...
  std::vector<uint128_t> ConvertShares(const std::vector<uint128_t>& items,
                                       size_t count,
                                       const std::vector<Share>& shares) const;
  std::vector<uint128_t> Reconstruct(
      const std::vector<uint128_t>& items, size_t count,
      const std::vector<uint128_t>& shares) const;
  // std::vector<uint128_t> SwapShares(const std::vector<uint128_t>& items,
  //                  const std::vector<Share>& shares) const;
  // auto Reconstruct(const std::vector<uint128_t>& items,
