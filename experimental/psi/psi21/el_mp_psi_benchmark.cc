...
void ElMpPsiSend(const std::shared_ptr<yacl::link::Context>& link_ctx,
  ...
  return psi::psi::ElSend(link_ctx, items_hash, shares);
}

std::vector<uint128_t> ElMpPsiRecv(
    const std::shared_ptr<yacl::link::Context>& link_ctx,
    const std::vector<uint128_t>& items_hash) {
  return psi::psi::ElRecv(link_ctx, items_hash);
}

}  // namespace

static void BM_El_Mp_Psi(benchmark::State& state) {
  for (auto _ : state) {
    state.PauseTiming();
    ...
    std::future<void> kkrt_psi_sender =
        std::async([&] { return ElMpPsiSend(contexts[0], alice_items); });
    std::future<std::vector<uint128_t>> kkrt_psi_receiver =
        std::async([&] { return ElMpPsiRecv(contexts[1], bob_items); });
