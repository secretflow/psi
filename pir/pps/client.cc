...
void PpsPirClient::Setup(PIRKey& sk, PIREvalMap& map,
                         std::set<uint64_t>& deltas) {
  PPS pps(universe_size_, set_size_);
  sk = pps.Gen(map);
...
void PpsPirClient::Query(uint64_t i, PIRKey& sk, PIREvalMap& map,
  ...
  PPS pps(universe_size_, set_size_);
  pps.Punc(i_punc, sk, map, sk_punc);
}
...
void PpsPirClient::Query(uint64_t i, std::vector<PIRKeyUnion>& ck,
  ...
  PPS pps(universe_size_, set_size_);
  PIREvalMap eval_map;
  PIRKey k_new = pps.Gen(eval_map);
  PIRKeyUnion k_right;
  uint64_t rand_i = std::next(eval_map.begin(), UniformUint64())->first;
  punc_l.delta_ = MODULE_SUB(i, rand_i, universe_size_);

  uint64_t i_punc;
  param.j_ = PIR_ABORT;
  if ((param.b_ = Bernoulli())) {
    rand_i = std::next(eval_map.begin(), UniformUint64())->first;
  ...
  pps.Punc(i_punc_l, k_new, eval_map, punc_l);
  if (param.j_) {
    eval_map.clear();
    pps.EvalMap(k_right.k_, eval_map);
    uint64_t i_punc_r = MODULE_SUB(i_punc, k_right.delta_, universe_size_);
    pps.Punc(i_punc_r, k_right.k_, eval_map, punc_r);
