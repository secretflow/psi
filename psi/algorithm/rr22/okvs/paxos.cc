...
void Paxos<IdxType>::SetInput(absl::Span<const uint128_t> inputs) {
...

  {
    ...
    for (uint64_t i = 0; i < main;
      ...
      // return to DEBUG
      SPDLOG_DEBUG("i:{} cols size:{} sparse_size:{}, weight: {}", i,
                   cols.size(), sparse_size, weight);
      for (auto c : cols) {
        SPDLOG_DEBUG("c: {}, col_weights.size: {}", c, col_weights.size());
...
void Paxos<IdxType>::EncodeU64(
  ...
  BackfillU64(absl::MakeSpan(main_rows), absl::MakeSpan(main_cols),
...
template <typename IdxType>
void Paxos<IdxType>::BackfillU64(
  ...
  if (dt == DenseType::GF128) {
    BackfillGf128U64(main_rows, main_cols, gap_rows, values, output, h, prng);
  } else {
    BackfillBinaryU64(main_rows, main_cols, gap_rows, values, output, h, prng);
  }
...
template <typename IdxType>
void Paxos<IdxType>::BackfillGf128U64(
    absl::Span<IdxType> main_rows, absl::Span<IdxType> main_cols,
    absl::Span<std::array<IdxType, 2>> gap_rows, const PxVectorU64& values,
    PxVectorU64& output, PxVectorU64::Helper& helper,
    const std::shared_ptr<yacl::crypto::Prg<uint8_t>>& prng) {
  ...
  if (g) {
    ...
    // xx = x' - FC^-1 x
    PxVectorU64 xx = helper.NewVec(size);
...
template <typename IdxType>
void Paxos<IdxType>::BackfillBinaryU64(
    absl::Span<IdxType> main_rows, absl::Span<IdxType> main_cols,
    absl::Span<std::array<IdxType, 2>> gap_rows, const PxVectorU64& X,
    PxVectorU64& P, PxVectorU64::Helper& h,
    const std::shared_ptr<yacl::crypto::Prg<uint8_t>>& prng) {
  ...
  std::vector<uint64_t> dense_masks(gg);

  // the dense part of the paxos.
  auto p2 = P.subspan(sparse_size);

  YACL_ENFORCE(gg <= g, "gg:{}<=gg:{}", g, g);
  ...
  if (gg) {
    ...
    if (prng) {
      RandomizeDenseColsU64(p2, h, absl::MakeSpan(gap_cols), prng);
    }

    // auto PP =
    //  x2' = x2 - D r - FC^-1 x1
    auto xx2 =
        GetX2PrimeU64(fcinv, absl::MakeSpan(gap_rows), absl::MakeSpan(gap_cols),
                      X, prng ? P : PxVectorU64{}, h);
    ...
    for (uint64_t i = 0; i < gg; ++i) {
      auto pp = p2[gap_cols[i]];
      // if (pp != oc::ZeroBlock)
      //	throw RTE_LOC;

      for (uint64_t j = 0; j < gg; ++j) {
        if (EEInv(i, j)) {
          // pp = pp ^ xx2[j]
          h.Add(pp, xx2[j]);
        }
  ...
  for (uint64_t k = 0; k < main_rows.size(); ++k) {
    ...
    if (prng) {
      auto d = yacl::DecomposeUInt128(dense_[i]).second;
      ...
      for (uint64_t j = 0; j < gg; ++j) {
        ...
        if (yacl::DecomposeUInt128(dense_[i]).second & dense_masks[j]) {
...
void Paxos<IdxType>::BackfillBinary(
  ...
  for (uint64_t k = 0; k < main_rows.size(); ++k) {
    ...
    // y = X[i]
    h.Assign(y, X[i]);
    // note the rows_ is different from volepsi, here we need consider the
    // stride
    auto row = &rows_[i * weight];
    for (uint64_t j = 0; j < weight; ++j) {
      auto cc = row[j];
...
std::vector<uint64_t> Paxos<IdxType>::GetGapCols(
  ...
  // avoid using same variable name as PaxosParameter
  auto cur_g = gap_rows.size();
  uint64_t ci = 0;
  uint64_t e = Choose(dense_size, cur_g);
  ...
  while (true) {
    ...
    auto gap_cols = ithCombination(ci, dense_size, cur_g);
    ++ci;
    YACL_ENFORCE(ci <= e, "failed to find invertible matrix. ci:{}, e:{}", ci,
                 e);

    EE.resize(cur_g, cur_g);

    for (uint64_t i = 0; i < cur_g; ++i) {
...
template <typename IdxType>
PxVectorU64 Paxos<IdxType>::GetX2PrimeU64(
    const FCInv& fcinv, absl::Span<std::array<IdxType, 2>> gap_rows,
    absl::Span<uint64_t> gap_cols, const PxVectorU64& X, const PxVectorU64& P,
    PxVectorU64::Helper& helper) {
  YACL_ENFORCE(X.size() == num_items_);
  bool randomized = P.size() != 0;

  auto g = gap_rows.size();
  PxVectorU64 xx2 = helper.NewVec(g);
...
template <typename IdxType>
void Paxos<IdxType>::RandomizeDenseColsU64(
    PxVectorU64& p2, PxVectorU64::Helper& h, absl::Span<uint64_t> gap_cols,
    const std::shared_ptr<yacl::crypto::Prg<uint8_t>>& prng) {
  YACL_ENFORCE(prng);
...
template <typename IdxType>
void Paxos<IdxType>::DecodeU64(absl::Span<const uint128_t> inputs,
                               absl::Span<uint64_t> values,
                               absl::Span<uint64_t> p) {
  PxVectorU64 VV(values);
  PxVectorU64 PP(p);
  auto h = PP.DefaultHelper();
  DecodeU64(inputs, VV, PP, h);
}

template <typename IdxType>
void Paxos<IdxType>::DecodeU64(absl::Span<const uint128_t> inputs,
                               PxVectorU64& values, const PxVectorU64& PP,
  ...
  if (add_to_decode_) {
    PxVectorU64 v = h.NewVec(kPaxosBuildRowSize);
    ...
    for (uint64_t i = 0; i < batch_count;
      ...
      Decode32U64(absl::MakeSpan(rows), absl::MakeSpan(dense),
    ...
    for (uint64_t i = batch_count; i < inputs.size(); ++i, ++in_iter) {
      hasher_.HashBuildRow1(*in_iter, absl::MakeSpan(rows.data(), weight),
                            &dense[0]);
      Decode1U64(absl::MakeSpan(rows.data(), weight), dense[0], v[0], PP, h);
      h.Add(values[i], v[0]);
    }
  } else {
    for (uint64_t i = 0; i < batch_count;
    ...
    for (uint64_t i = batch_count; i < inputs.size(); ++i, ++in_iter) {
      hasher_.HashBuildRow1(*in_iter, absl::MakeSpan(rows.data(), weight),
                            &dense[0]);
      Decode1U64(absl::MakeSpan(rows.data(), weight), dense[0], values[i], PP,
                 h);
    }
  }
...
template <typename IdxType>
void Paxos<IdxType>::Decode32U64(absl::Span<IdxType> rows_span,
                                 absl::Span<uint128_t> dense_span,
                                 absl::Span<uint64_t> values_span,
                                 const PxVectorU64& p_,
                                 const PxVectorU64::Helper& h) {
  const uint64_t* __restrict p = p_[0];

  for (uint64_t j = 0; j < 4; ++j) {
    const IdxType* __restrict rows = rows_span.data() + j * 8 * weight;
    uint64_t* __restrict values = h.IterPlus(values_span.data(), j * 8);
  ...
  for (uint64_t j = 1; j < weight; ++j) {
    for (uint64_t k = 0; k < 4; ++k) {
      ...
      uint64_t* __restrict values = h.IterPlus(values_span.data(), k * 8);
  ...
  if (dt == DenseType::GF128) {
    const uint64_t* __restrict p2 = h.IterPlus(p, sparse_size);
...
// the value op. helper.
template <typename IdxType>
void Paxos<IdxType>::Decode1U64(absl::Span<IdxType> rows, const uint128_t dense,
