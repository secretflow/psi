...
uint32_t ReverseBits(uint32_t num, uint32_t msb) {
  ...
  switch (msbb) {
    ...
    case 2:
      return (ReverseByte(num & 0xff) << 8 | ReverseByte((num >> 8) & 0xff)) >>
             shift_trick[msb & 0x7];
    case 3:
...
MatVecU8U32Mod2p32Avx512(const uint8_t* A, const uint32_t* x, uint32_t* y,
...
__attribute__((target("avx512f,avx512bw,avx512vl"))) void
MatrixVectorMultiplicationU16Avx512(
...
void PseudorandomMatrixGenerate(std::vector<std::vector<uint64_t>>& matrix,
  ...
  for (uint64_t i = 0; i < rows; ++i)
    for (uint64_t j = 0; j < cols; ++j) matrix[i][j] %= modulus;
}
...
void SetPseudorandomHypercubeYpir(
  ...
  cube.assign(layers, std::vector<std::vector<uint64_t>>(
                          rows, std::vector<uint64_t>(cols)));
