...
static void BM_LabeledPsi(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::shared_ptr<ISenderDB> sender_db = std::make_shared<SenderKvDB>(
        psi_params, oprf_key, "::memory", 0, nonce_byte_count, compressed);
    ...
    std::shared_ptr<IBatchProvider> batch_provider =
        std::make_shared<MemoryBatchProvider>(sender_items, 5000);
    ...
    std::future<std::pair<std::vector<::apsi::HashedItem>,
