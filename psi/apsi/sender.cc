...
LabelPsiSender::LabelPsiSender(std::shared_ptr<ISenderDB> sender_db)
...
void LabelPsiSender::RunOPRF(
    const std::shared_ptr<ecdh::IEcdhOprfServer> &oprf_server,
...
std::vector<std::shared_ptr<ResultPackage>> SenderRunQuery(
    const QueryRequest &query, const std::shared_ptr<ISenderDB> &sender_db,
...
void LabelPsiSender::RunQuery(
  ...
  std::unordered_map<uint32_t,
                     std::vector<::apsi::SEALObject<seal::Ciphertext>>>
      encrypted_powers;
...
void ComputePowers(const std::shared_ptr<ISenderDB> &sender_db,
