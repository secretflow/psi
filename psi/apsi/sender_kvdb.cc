...
PreprocessLabeledData(const std::pair<::apsi::HashedItem,
                                      ::apsi::EncryptedLabel> &item_label_pair,
...
void InsertOrAssignWorker(
  ...
  for (size_t i = 0; i < indices_count; ++i) {
    ...
    YACL_ENFORCE(get_status, "get_status:{}", get_status);
...
std::shared_ptr<::apsi::sender::BinBundle> SenderKvDB::GetBinBundleAt(
  ...
  YACL_ENFORCE(load_ret.first == cache_idx);
...
void SenderKvDB::InsertOrAssign(
  ...
  while (true) {
    ...
    if (IsLabeled()) {
      std::shared_ptr<ILabeledBatchProvider> labeled_batch_provider =
          std::dynamic_pointer_cast<ILabeledBatchProvider>(batch_provider);

      std::tie(batch_items, batch_labels) =
          labeled_batch_provider->ReadNextLabeledBatch();
    } else {
      std::shared_ptr<IBasicBatchProvider> basic_batch_provider =
          std::dynamic_pointer_cast<IBasicBatchProvider>(batch_provider);

      batch_items = basic_batch_provider->ReadNextBatch();
    }
    ...
    if (IsLabeled()) {
      ...
      // Set up Kuku hash functions
      auto hash_funcs = HashFunctions(params_);
      ...
      yacl::parallel_for(0, oprf_out.size(), [&](int64_t begin, int64_t end) {
        for (int64_t idx = begin; idx < end; ++idx) {
          ...
          std::pair<::apsi::HashedItem, ::apsi::EncryptedLabel>
              item_label_pair = std::make_pair(hashed_item, encrypted_label);
      ...
      for (size_t i = 0; i < oprf_out.size(); ++i) {
        ...
        std::vector<std::pair<::apsi::util::AlgItem, size_t>>
            data_with_indices = PreprocessUnlabeledData(hashed_item, params_);
