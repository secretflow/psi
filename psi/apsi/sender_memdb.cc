...
PreprocessLabeledData(
  ...
  for (auto it = begin; it != end; it++) {
    const std::pair<::apsi::HashedItem, ::apsi::EncryptedLabel>
        &item_label_pair = *it;
...
void SenderMemDB::InsertOrAssign(const std::vector<std::string> &keys,
  ...
  std::vector<std::pair<::apsi::HashedItem, ::apsi::EncryptedLabel>>
      hashed_data(oprf_out.size());
  ...
  if (new_item_count != 0) {
    ...
    std::vector<std::pair<::apsi::util::AlgItemLabel, size_t>>
        data_with_indices = PreprocessLabeledData(hashed_data.begin(),
