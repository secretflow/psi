...
source_file="${REPO_DIR}/psi/version.h"
...
# OPENSOURCE-CLEANUP SUB registry.cn-hangzhou.aliyuncs.com/oasis-stack secretflow
DOCKER_REG="registry.cn-hangzhou.aliyuncs.com/oasis-stack"

IMAGE_TAG=${DOCKER_REG}/psi_pir-anolis8:${VERSION}
LATEST_TAG=${DOCKER_REG}/psi_pir-anolis8:latest
...
if [[ SKIP -eq 0 ]]; then
    ...
    docker run -it  --rm   --mount type=bind,source="${REPO_DIR}",target=/home/admin/dev/src -w /home/admin/dev  --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --cap-add=NET_ADMIN --privileged=true reg.docker.alibaba-inc.com/secretflow/release-ci:latest /home/admin/dev/src/psi/apsi_wrapper/docker/entry.sh
