...
void Receiver::reset_keys() {
  // Generate new keys
  ::seal::KeyGenerator generator(*get_seal_context());
...
uint32_t Receiver::reset_powers_dag(const set<uint32_t> &source_powers) {
  ...
  set<uint32_t> target_powers =
      ::apsi::util::create_powers_set(params_.query_params().ps_low_degree,
                                      params_.table_params().max_items_per_bin);
  ...
  if (!pd_.is_configured()) {
    APSI_LOG_ERROR(
        ...
        << "source_powers: " << ::apsi::util::to_string(source_powers) << ", "
        << "target_powers: " << ::apsi::util::to_string(target_powers) << ")");
...
void Receiver::initialize() {
  ...
  STOPWATCH(::apsi::util::recv_stopwatch, "Receiver::initialize");

  // Initialize the CryptoContext with a new SEALContext
  crypto_context_ = ::apsi::CryptoContext(params_);
...
unique_ptr<::apsi::network::SenderOperation> Receiver::CreateParamsRequest() {
  auto sop = make_unique<::apsi::network::SenderOperationParms>();
  APSI_LOG_INFO("Created parameter request");

  return sop;
}

::apsi::PSIParams Receiver::RequestParams(
  ...
  ::apsi::ParamsResponse response;
  bool logged_waiting = false;
  while (!(response = ::apsi::to_params_response(chl.receive_response()))) {
    if (!logged_waiting) {
...
::apsi::oprf::OPRFReceiver Receiver::CreateOPRFReceiver(
    const vector<::apsi::Item> &items) {
  STOPWATCH(::apsi::util::recv_stopwatch, "Receiver::CreateOPRFReceiver");

  ::apsi::oprf::OPRFReceiver oprf_receiver(items);
...
pair<vector<::apsi::HashedItem>, vector<::apsi::LabelKey>>
Receiver::ExtractHashes(const ::apsi::OPRFResponse &oprf_response,
                        const ::apsi::oprf::OPRFReceiver &oprf_receiver) {
  STOPWATCH(::apsi::util::recv_stopwatch, "Receiver::ExtractHashes");
  ...
  size_t oprf_response_item_count =
...
unique_ptr<::apsi::network::SenderOperation> Receiver::CreateOPRFRequest(
    const ::apsi::oprf::OPRFReceiver &oprf_receiver, std::uint32_t bucket_idx) {
  auto sop = make_unique<::apsi::network::SenderOperationOPRF>();
...
pair<vector<::apsi::HashedItem>, vector<::apsi::LabelKey>>
Receiver::RequestOPRF(const vector<::apsi::Item> &items,
                      ::apsi::network::NetworkChannel &chl,
...
pair<::apsi::Request, ::apsi::receiver::IndexTranslationTable>
Receiver::create_query(const vector<::apsi::HashedItem> &items,
                       std::uint32_t bucket_idx) {
  APSI_LOG_INFO("Creating encrypted query for " << items.size() << "items");
  STOPWATCH(::apsi::util::recv_stopwatch, "Receiver::create_query");
  ...
  // cuckoo_hashing
  {
    STOPWATCH(::apsi::util::recv_stopwatch,
              "Receiver::create_query::cuckoo_hashing");
    APSI_LOG_DEBUG("Inserting "
                   << items.size() << " items into cuckoo table of size "
    ...
    for (size_t item_idx = 0; item_idx < items.size(); item_idx++) {
      ...
      if (!cuckoo.insert(item.get_as<::kuku::item_type>().front())) {
        ...
        if (cuckoo.is_empty_item(cuckoo.leftover_item())) {
          APSI_LOG_INFO("Skipping repeated insertion of items["
                        << item_idx << "]: " << item.to_string());
        } else {
          APSI_LOG_ERROR("Failed to insert items["
                         << item_idx << "]: " << item.to_string()
    ...
    APSI_LOG_DEBUG("Finished inserting items with "
                   << cuckoo.loc_func_count()
                   << " hash functions; cuckoo table fill-rate: "
                   << cuckoo.fill_rate());
  }
  ...
  for (size_t item_idx = 0; item_idx < items.size(); item_idx++) {
    auto item_loc =
        cuckoo.query(items[item_idx].get_as<::kuku::item_type>().front());
    itt.table_idx_to_item_idx_[item_loc.location()] = item_idx;
  }

  // Set up unencrypted query data
  vector<::apsi::receiver::PlaintextPowers> plain_powers;
  ...
  // prepare_data
  {
    ...
    for (uint32_t bundle_idx = 0; bundle_idx < params_.bundle_idx_count();
      ...
      gsl::span<const ::kuku::item_type> bundle_items(
      ...
      for (auto &item : bundle_items) {
        ...
        // plain_modulus parts
        vector<uint64_t> alg_item = ::apsi::util::bits_to_field_elts(
            item_bits, params_.seal_params().plain_modulus());
  ...
  unordered_map<uint32_t, vector<::apsi::SEALObject<::seal::Ciphertext>>>
      encrypted_powers;

  // encrypt_data
  {
    STOPWATCH(::apsi::util::recv_stopwatch,
              "Receiver::create_query::encrypt_data");
    for (uint32_t bundle_idx = 0; bundle_idx < params_.bundle_idx_count();
         bundle_idx++) {
...
vector<::apsi::receiver::MatchRecord> Receiver::request_query(
    const vector<::apsi::HashedItem> &items,
    const vector<::apsi::LabelKey> &label_keys,
    ::apsi::network::NetworkChannel &chl, bool streaming_result,
    std::uint32_t bucket_idx) {
  ...
  if (streaming_result) {
    // Set up the result
    vector<::apsi::receiver::MatchRecord> mrs(query.second.item_count());
    ...
    size_t task_count =
        min<size_t>(::apsi::ThreadPoolMgr::GetThreadCount(), package_count);
...
vector<::apsi::receiver::MatchRecord> Receiver::process_result_part(
    const vector<::apsi::LabelKey> &label_keys,
    const ::apsi::receiver::IndexTranslationTable &itt,
    const ::apsi::ResultPart &result_part) const {
  STOPWATCH(::apsi::util::recv_stopwatch, "Receiver::process_result_part");
  ...
  ::apsi::network::PlainResultPackage plain_rp =
      result_part->extract(crypto_context_);

  size_t felts_per_item =
      ::seal::util::safe_cast<size_t>(params_.item_params().felts_per_item);
  size_t items_per_bundle =
      ::seal::util::safe_cast<size_t>(params_.items_per_bundle());
  size_t bundle_start = ::seal::util::mul_safe(
      ::seal::util::safe_cast<size_t>(plain_rp.bundle_idx), items_per_bundle);

  // Check if we are supposed to have label data present but don't have for some
  // reason
  ...
  if (label_byte_count && plain_rp.label_result.empty()) {
    APSI_LOG_WARNING("Expected " << label_byte_count
  ...
  size_t received_label_bit_count = ::seal::util::mul_safe(
      ::seal::util::safe_cast<size_t>(params_.item_bit_count()),
  ...
  if (label_byte_count && label_keys.size() != item_count) {
    APSI_LOG_WARNING("Expected " << item_count
                                 << " label encryption keys but only "
                                 << label_keys.size()
  ...
  vector<::apsi::receiver::MatchRecord> mrs(item_count);
  ...
  seal_for_each_n(
      iter(plain_rp_iter, size_t(0)), items_per_bundle, [&](auto &&I) {
        ...
        // Compute the cuckoo table index for this item. Then find the
        // corresponding index in the input items vector so we know where to
        // place the result.
        size_t table_idx = ::seal::util::add_safe(get<1>(I), bundle_start);
        auto item_idx = itt.find_item_idx(table_idx);

        // If this table_idx doesn't match any item_idx, ignore the result no
        // matter what it is
        ...
        if (mrs[item_idx]) {
          APSI_LOG_ERROR(
              ...
              "result package; the translation table (query) has probably been "
              "corrupted");
          throw runtime_error(
              "found a duplicate positive match; something is seriously wrong");
        }

        APSI_LOG_DEBUG("Match found for items["
                       << item_idx << "] at cuckoo table index " << table_idx);

        // Create a new MatchRecord
        ::apsi::receiver::MatchRecord mr;
        mr.found = true;

        // Next, extract the label results, if any
        if (label_byte_count) {
          APSI_LOG_DEBUG("Found " << plain_rp.label_result.size()
                                  << " label parts for items[" << item_idx
                                  << "]; expecting " << label_byte_count
                                  << "-byte label");
          ...
          size_t label_offset =
              ::seal::util::mul_safe(get<1>(I), felts_per_item);
          for (auto &label_parts : plain_rp.label_result) {
            ...
            copy(label_part.begin(), label_part.end(),
          ...
          // Decrypt the label
          apsi::Label label = ::apsi::util::decrypt_label(
...
vector<::apsi::receiver::MatchRecord> Receiver::process_result(
    const vector<::apsi::LabelKey> &label_keys,
    const ::apsi::receiver::IndexTranslationTable &itt,
    const vector<::apsi::ResultPart> &result) const {
  APSI_LOG_INFO("Processing " << result.size() << " result parts");
  STOPWATCH(::apsi::util::recv_stopwatch, "Receiver::process_result");

  vector<::apsi::receiver::MatchRecord> mrs(itt.item_count());
          ...
          if (get<1>(I) && !get<0>(I)) {
            ...
            APSI_LOG_ERROR(
                ...
                << "] but an existing match for this location was "
                   "already found before from a different result part");

            throw runtime_error(
                "found a duplicate positive match; something is seriously "
                "wrong");
          }
        });
  }
...
void Receiver::process_result_worker(
    atomic<uint32_t> &package_count, vector<::apsi::receiver::MatchRecord> &mrs,
    const vector<::apsi::LabelKey> &label_keys,
    const ::apsi::receiver::IndexTranslationTable &itt,
    ::apsi::network::Channel &chl) const {
  stringstream sw_ss;
  sw_ss << "Receiver::process_result_worker [" << this_thread::get_id() << "]";
  STOPWATCH(::apsi::util::recv_stopwatch, sw_ss.str());
  ...
  while (true) {
    ...
    if (!package_count.compare_exchange_strong(curr_package_count,
                                               curr_package_count - 1)) {
      continue;
    }
