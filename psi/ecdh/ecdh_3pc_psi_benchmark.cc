...
static void BM_Ecdh3PcPsi(benchmark::State& state) {
  for (auto _ : state) {
    ...
    auto alice_items = psi::test::CreateRangeItems(1, n);
    auto bob_items = psi::test::CreateRangeItems(2, n);
    auto carol_items = psi::test::CreateRangeItems(3, n);

    auto contexts = yacl::link::test::SetupWorld(3);

    // simple runner
    auto psi_func =
        [&](const std::shared_ptr<psi::ecdh::ShuffleEcdh3PcPsi>& handler,
    ...
    auto alice_func = std::async([&]() {
      psi::ecdh::ShuffleEcdh3PcPsi::Options opts;
      opts.link_ctx = contexts[0];
      opts.master_rank = 0;
      auto op = std::make_shared<psi::ecdh::ShuffleEcdh3PcPsi>(opts);
