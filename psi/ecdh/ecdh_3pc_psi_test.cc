...
TEST_P(Ecdh3PcPsiTest, PartnersPsi) {
  ...
  auto intersection_std_bc =
      test::GetIntersection(params.items_b, params.items_c);
...
TEST_P(Ecdh3PcPsiTest, Works) {
  ...
  auto intersection_std_abc =
      test::GetIntersection(intersection_std_ab, params.items_c);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(TestParams{{"a", "b"}, {"b", "c"}, {"b", "d"}},  //
                    ...
                    TestParams{test::CreateRangeItems(0, 4095),
                               test::CreateRangeItems(1, 4095),
                               test::CreateRangeItems(2, 4095)},  //

                    // exactly one batch
                    TestParams{test::CreateRangeItems(0, 4096),
                               test::CreateRangeItems(1, 4096),
                               test::CreateRangeItems(2, 4096)},  //
                    // more than one batch
                    TestParams{test::CreateRangeItems(0, 8193),
                               test::CreateRangeItems(5, 8193),
                               test::CreateRangeItems(10, 8193)},  //
                    //
                    TestParams{{}, {}, {}}  //
                    ));
