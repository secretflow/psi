               ...
               stop_flag)
  {
    ...
    while (!stop_flag) {
      ...
      std::tie(batch_items, batch_indices, shuffle_indices) =
          batch_provider->ReadNextShuffledBatch();
...
void EcdhOprfPsiServer::RecvBlindAndShuffleSendEvaluate() {
  ...
  std::unique_ptr<IBasicBatchProvider> provider =
      std::make_unique<MemoryBatchProvider>(evaluated_items,
                                            options_.batch_size);

  batch_count = 0;
  while (true) {
    std::vector<std::string> batch_evaluated_items = provider->ReadNextBatch();
...
EcdhOprfPsiServer::RecvIntersectionMaskedItems(
  ...
  while (true) {
    ...
    std::tie(server_masked_items, batch_indices, batch_shuffled_indices) =
        cache_provider->ReadNextShuffledBatch();
...
void EcdhOprfPsiClient::RecvFinalEvaluatedItems(
  ...
  while (true) {
    ...
    peer_ec_point_store->Save(evaluated_items);
...
void EcdhOprfPsiClient::RecvEvaluatedItems(
  ...
  while (true) {
    ...
    self_ec_point_store->Save(oprf_items);
