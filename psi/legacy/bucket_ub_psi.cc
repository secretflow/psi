...
std::pair<std::vector<uint64_t>, size_t> UbPsi(
  ...
  ecdh::EcdhOprfPsiOptions psi_options;
  ...
  switch (config.psi_type()) {
    ...
    case PsiType::ECDH_OPRF_UB_PSI_2PC_ONLINE:
      ...
      break;
    default:
      YACL_THROW("Invalid unbalanced psi subprotocol: {}", config.psi_type());
  }
...
std::pair<std::vector<uint64_t>, size_t> UbPsiServerGenCache(
    BucketPsiConfig config, std::shared_ptr<yacl::link::Context> /*lctx*/,
    const ecdh::EcdhOprfPsiOptions& psi_options) {
  std::vector<uint8_t> server_private_key =
      ReadEcSecretKeyFile(config.ecdh_secret_key_path());

  std::shared_ptr<ecdh::EcdhOprfPsiServer> dh_oprf_psi_server =
      std::make_shared<ecdh::EcdhOprfPsiServer>(psi_options,
                                                server_private_key);
...
std::pair<std::vector<uint64_t>, size_t> UbPsiClientTransferCache(
    BucketPsiConfig config, std::shared_ptr<yacl::link::Context> lctx,
    const ecdh::EcdhOprfPsiOptions& psi_options) {
  std::shared_ptr<ecdh::EcdhOprfPsiClient> ub_psi_client_transfer_cache =
      std::make_shared<ecdh::EcdhOprfPsiClient>(psi_options);
...
std::pair<std::vector<uint64_t>, size_t> UbPsiServerTransferCache(
    BucketPsiConfig config, std::shared_ptr<yacl::link::Context> lctx,
    const ecdh::EcdhOprfPsiOptions& psi_options) {
  std::shared_ptr<ecdh::EcdhOprfPsiServer> ub_psi_server_transfer_cache =
      std::make_shared<ecdh::EcdhOprfPsiServer>(psi_options);

  std::shared_ptr<IBasicBatchProvider> batch_provider =
      std::make_shared<UbPsiCacheProvider>(
          config.input_params().path(), psi_options.batch_size,
...
std::pair<std::vector<uint64_t>, size_t> UbPsiClientShuffleOnline(
    BucketPsiConfig config, std::shared_ptr<yacl::link::Context> lctx,
    const ecdh::EcdhOprfPsiOptions& psi_options, const std::string& tmp_dir) {
  std::vector<uint8_t> private_key = yacl::crypto::RandBytes(kEccKeySize);

  std::shared_ptr<ecdh::EcdhOprfPsiClient> ub_psi_client_shuffle_online =
      std::make_shared<ecdh::EcdhOprfPsiClient>(psi_options, private_key);
  ...
  auto peer_ec_point_store = std::make_shared<CachedCsvEcPointStore>(
      config.preprocess_path(), false, "peer", true);

  SPDLOG_INFO("shuffle online protocol CachedCsvCipherStore: {} {}",
              ec_point_store_path1, config.preprocess_path());

  size_t self_items_count =
      ub_psi_client_shuffle_online->SendBlindedItems(batch_provider);

  ub_psi_client_shuffle_online->RecvEvaluatedItems(self_ec_point_store);

  self_ec_point_store->Flush();

  std::vector<uint64_t> indices;
  std::vector<std::string> masked_items;
  std::tie(indices, masked_items) = FinalizeAndComputeIndices(
      self_ec_point_store, peer_ec_point_store, config.bucket_size());
  SPDLOG_INFO("indices size:{}", indices.size());

  std::shared_ptr<IBasicBatchProvider> intersection_masked_provider =
...
std::pair<std::vector<uint64_t>, size_t> UbPsiServerShuffleOnline(
  ...
  std::shared_ptr<ecdh::EcdhOprfPsiServer> ub_psi_server_shuffle_online =
      std::make_shared<ecdh::EcdhOprfPsiServer>(psi_options,
                                                server_private_key);

  ub_psi_server_shuffle_online->RecvBlindAndShuffleSendEvaluate();

  std::shared_ptr<IShuffledBatchProvider> cache_provider =
      std::make_shared<UbPsiCacheProvider>(
          config.preprocess_path(), config.bucket_size(),
          ub_psi_server_shuffle_online->GetCompareLength());

  size_t self_items_size;
  std::vector<uint64_t> results;
  std::tie(results, self_items_size) =
      ub_psi_server_shuffle_online->RecvIntersectionMaskedItems(cache_provider);
...
std::pair<std::vector<uint64_t>, size_t> UbPsiServerOffline(
  ...
  std::shared_ptr<ecdh::EcdhOprfPsiServer> dh_oprf_psi_server_offline =
      std::make_shared<ecdh::EcdhOprfPsiServer>(psi_options,
                                                server_private_key);
...
std::pair<std::vector<uint64_t>, size_t> UbPsiClientOnline(
    BucketPsiConfig config, std::shared_ptr<yacl::link::Context> lctx,
    const ecdh::EcdhOprfPsiOptions& psi_options, const std::string& tmp_dir) {
  std::shared_ptr<ecdh::EcdhOprfPsiClient> dh_oprf_psi_client_online =
      std::make_shared<ecdh::EcdhOprfPsiClient>(psi_options);
...
std::pair<std::vector<uint64_t>, size_t> UbPsiServerOnline(
  ...
  std::shared_ptr<ecdh::EcdhOprfPsiServer> dh_oprf_psi_server_online =
      std::make_shared<ecdh::EcdhOprfPsiServer>(psi_options,
                                                server_private_key);
