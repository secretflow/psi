...
void WriteSecretKey(const std::string &ecdh_secret_key_path) {
  ...
  (void)absl::HexStringToBytes(
      "000102030405060708090a0b0c0d0e0ff0e0d0c0b0a090807060504030201000", &secret_key_binary);
...
TEST_P(UnbalancedPsiTest, Work) {
  ...
  // step 1, gen cache
  {
    ...
    config.set_psi_type(PsiType::ECDH_OPRF_UB_PSI_2PC_GEN_CACHE);
  ...
  auto transfer_cache_proc = [&](int idx) -> PsiResultReport {
    BucketPsiConfig config;
    ...
    config.mutable_output_params()->set_need_sort(false);
    config.set_psi_type(PsiType::ECDH_OPRF_UB_PSI_2PC_TRANSFER_CACHE);
  ...
  std::vector<std::future<PsiResultReport>> transfer_cache_f(world_size);
  ...
  PsiType psi_protocol = PsiType::ECDH_OPRF_UB_PSI_2PC_ONLINE;
  if (params.shuffle_online) {
    psi_protocol = PsiType::ECDH_OPRF_UB_PSI_2PC_SHUFFLE_ONLINE;
    receiver_rank = server_rank;
  }
  // online phase
  auto online_proc = [&](int idx) -> PsiResultReport {
  ...
  std::vector<std::future<PsiResultReport>> online_f(world_size);
  for (size_t i = 0; i < world_size; i++) {
