...
#include "psi/legacy/dp_2party_psi.h"
...
DpPsiOperator::DpPsiOperator(const std::shared_ptr<yacl::link::Context>& lctx,
                             const dp_psi::DpPsiOptions& options,
                             size_t receiver_rank, CurveType curve_type)
...
std::vector<std::string> DpPsiOperator::OnRun(
  ...
  if (receiver_rank_ == link_ctx_->Rank()) {
    std::vector<size_t> dp_psi_result = dp_psi::RunDpEcdhPsiBob(
    ...
    dp_psi::RunDpEcdhPsiAlice(dp_options_, link_ctx_, inputs,
...
std::unique_ptr<PsiBaseOperator> CreateOperator(
  ...
  dp_psi::DpPsiOptions dp_options(bob_sub_sampling, epsilon);
