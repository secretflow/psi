...
#include "psi/legacy/kkrt_2party_psi.h"
...
std::vector<std::string> KkrtPsiOperator::OnRun(
  ...
  if (options_.receiver_rank == link_ctx_->Rank()) {
    ...
    auto ot_recv =
        kkrt::GetKkrtOtSenderOptions(options_.link_ctx, options_.num_ot);
    kkrt::KkrtPsiSend(options_.link_ctx, ot_recv, items_hash);
  }
