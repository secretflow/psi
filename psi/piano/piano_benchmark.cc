...
std::vector<uint64_t> CreateDatabase(const uint64_t db_size,
  ...
  const auto [ChunkSize, SetSize] = psi::piano::GenParams(db_size);
  std::vector<uint64_t> DB;
  DB.assign(ChunkSize * SetSize * psi::piano::DBEntryLength, 0);

  for (uint64_t i = 0; i < DB.size() / psi::piano::DBEntryLength; ++i) {
    auto entry = psi::piano::GenDBEntry(db_seed, i);
    std::memcpy(&DB[i * psi::piano::DBEntryLength], entry.data(),
                psi::piano::DBEntryLength * sizeof(uint64_t));
  }
...
void SetupAndRunServer(
    ...
    std::vector<uint64_t>& db) {
  const auto [ChunkSize, SetSize] = psi::piano::GenParams(db_size);
  psi::piano::QueryServiceServer server(db, server_context, SetSize, ChunkSize);
  server.Start(exit_signal.get_future());
}

std::vector<psi::piano::DBEntry> SetupAndRunClient(
    const uint64_t db_size, const uint64_t thread_num,
    const std::shared_ptr<yacl::link::Context>& client_context,
    const std::vector<uint64_t>& queries) {
  psi::piano::QueryServiceClient client(db_size, thread_num, client_context);
  client.FetchFullDB();
...
static void BM_PianoPir(benchmark::State& state) {
  for (auto _ : state) {
    state.PauseTiming();
    uint64_t db_size = state.range(0) / sizeof(psi::piano::DBEntry);
