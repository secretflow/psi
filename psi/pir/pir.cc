...
std::vector<uint8_t> ReadEcSecretKeyFile(const std::string &file_path) {
  ...
  std::vector<uint8_t> secret_key(::psi::psi::kEccKeySize);
...
PirResultReport LabeledPirServer(
  ...
  std::vector<std::shared_ptr<::psi::psi::ISenderDB>> sender_db(bucket_count);
  std::vector<std::shared_ptr<::psi::psi::LabelPsiSender>> sender(bucket_count);
