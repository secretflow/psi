...
TEST_P(SealPirTest, Works) {
  ...
  psi::pir::SealPirOptions options{n, params.element_number,
                                   params.element_size, params.query_size};

  psi::pir::SealPirClient client(options);

  std::shared_ptr<IDbPlaintextStore> plaintext_store =
      std::make_shared<MemoryDbPlaintextStore>();
#ifdef DEC_DEBUG_
  psi::pir::SealPirServer server(options, client, plaintext_store);
#else
  psi::pir::SealPirServer server(options, plaintext_store);
#endif
