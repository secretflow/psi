...
#include "psi/psi/benchmark/mparty_bench.h"
...
DEFINE_uint32(rank, 0, "self rank, starts with 0");

DEFINE_string(parties, "",
...
void PreparePsiBench(const uint32_t rank, const std::string& parties) {
  ...
  if (parties.empty()) {
    ...
    host_ips = absl::StrSplit(psi::psi::bench::kTwoPartyHosts, ',');
  } else {
  ...
  psi::psi::bench::PsiBench::bench_lctx->ConnectToMesh();
}
}  // namespace
...
int main(int argc, char** argv) {
  ...
  try {
    PreparePsiBench(FLAGS_rank, FLAGS_parties);
    ...
    // sync close
    psi::psi::bench::PsiBench::bench_lctx->WaitLinkTaskFinish();
