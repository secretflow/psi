...
#include "psi/psi/core/bc22_psi/bc22_psi.h"
  ...
  (benchmark::State & state) {                                                 \
    for (auto _ : state) {                                                     \
      ...
      if (bench_lctx->Rank() == 0) {                                           \
        ...
        offline_proc.RecvFinalEvaluatedItems(peer_ec_point_store);             \
                                                                               \
        /* online */                                                           \
        auto proc_send = std::async([&] {                                      \
          auto item_provider = std::make_shared<MemoryBatchProvider>(          \
              items, kEcdhOprfPsiBatchSize);                                   \
          online_proc.SendBlindedItems(item_provider);                         \
        });                                                                    \
                                                                               \
        auto proc_recv = std::async(                                           \
            [&] { online_proc.RecvEvaluatedItems(self_ec_point_store); });     \
                                                                               \
        proc_send.get();                                                       \
        proc_recv.get();                                                       \
                                                                               \
        ...
        auto& self_results = self_ec_point_store->content();                   \
