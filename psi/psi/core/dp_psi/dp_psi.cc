...
#include "psi/psi/core/dp_psi/dp_psi_utils.h"
#include "psi/psi/core/ecdh_3pc_psi.h"
...
std::vector<size_t> BernoulliSamples(const std::vector<size_t>& items_idx,
  ...
  std::pair<uint64_t, uint64_t> seed_pair =
      yacl::DecomposeUInt128(yacl::crypto::SecureRandU128());
...
size_t RunDpEcdhPsiAlice(const DpPsiOptions& dp_psi_options,
  ...
  auto batch_provider =
      std::make_shared<MemoryBatchProvider>(items, kEcdhPsiBatchSize);
  ...
  std::future<void> f_recv_peer_a =
      std::async([&] { return psi_ctx.MaskPeer(peer_ec_point_store); });
  ...
  std::vector<std::string>& alice_peer_result = peer_ec_point_store->content();
...
std::vector<size_t> RunDpEcdhPsiBob(
  ...
  *sub_sample_size = items.size() - sub_sample_result.first.size();
  ...
  auto batch_provider = std::make_shared<MemoryBatchProvider>(
      sub_sample_result.first, kEcdhPsiBatchSize);
  ...
  std::vector<std::string>& bob_peer_result = peer_ec_point_store->content();
