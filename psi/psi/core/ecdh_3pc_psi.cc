...
void EcdhP2PExtendCtx::MaskSendSelf(
  ...
  auto batch_provider =
      std::make_shared<MemoryBatchProvider>(self_items, kEcdhPsiBatchSize);
...
void EcdhP2PExtendCtx::MaskRecvPeer(
  ...
  MaskPeer(peer_ec_point_store);

  *dup_masked_peer_items = peer_ec_point_store->content();
}
...
void EcdhP2PExtendCtx::RecvDupMasked(
  ...
  RecvDualMaskedSelf(self_ec_point_store);

  *dup_masked_items = self_ec_point_store->content();
}
...
void EcdhP2PExtendCtx::SendImpl(const std::vector<std::string>& items,
  ...
  while (true) {
    size_t curr_step_item_num =
        std::min(kEcdhPsiBatchSize, items.size() - send_count);
...
ShuffleEcdh3PcPsi::ShuffleEcdh3PcPsi(Options options)
  ...
  YACL_ENFORCE(RAND_bytes(private_key_.data(), kKeySize) == 1,
