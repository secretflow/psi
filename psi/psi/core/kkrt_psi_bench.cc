...
void KkrtPsiSend(const std::shared_ptr<yacl::link::Context>& link_ctx,
  ...
  auto ot_recv = psi::psi::GetKkrtOtSenderOptions(link_ctx, 512);
  return psi::psi::KkrtPsiSend(link_ctx, ot_recv, items_hash);
}

std::vector<std::size_t> KkrtPsiRecv(
    const std::shared_ptr<yacl::link::Context>& link_ctx,
    const std::vector<uint128_t>& items_hash) {
  auto ot_send = psi::psi::GetKkrtOtReceiverOptions(link_ctx, 512);
  return psi::psi::KkrtPsiRecv(link_ctx, ot_send, items_hash);
}
