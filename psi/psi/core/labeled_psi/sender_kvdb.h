...
#include "psi/psi/core/ecdh_oprf/basic_ecdh_oprf.h"
...
class SenderKvDB : public ISenderDB {
  ...
  */
  void SetData(const std::vector<std::string> &keys,
    ...
    std::shared_ptr<IBatchProvider> batch_provider =
        std::make_shared<MemoryBatchProvider>(keys, batch_size, labels);
  ...
  */
  void SetData(const std::vector<std::string> &data,
               size_t batch_size = 500000) {
    std::shared_ptr<IBatchProvider> batch_provider =
        std::make_shared<MemoryBatchProvider>(data, batch_size);

    SetData(batch_provider);
  }

  void SetData(const std::shared_ptr<IBatchProvider> &batch_provider) override {
    clear();
    InsertOrAssign(batch_provider);
  }
