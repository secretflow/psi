...
// psi demo
//  -- sender
// ./bazel-bin/psi/psi/core/mini_psi_demo --in ./100m/psi_1.csv --local
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (FLAGS_protocol == "semi-honest") {
      intersection = psi::psi::RunEcdhPsi(link_ctx, items, 1);
      if (FLAGS_rank == 1) {
      ...
      if (FLAGS_role == 0) {
        psi::psi::MiniPsiSendBatch(link_ctx, items);
      } else if (FLAGS_role == 1) {
        intersection = psi::psi::MiniPsiRecvBatch(link_ctx, items);
        SPDLOG_INFO("intersection size:{}", intersection.size());
        WriteCsvData(file_name, intersection);
      }
    }
