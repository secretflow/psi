...
#include "psi/psi/utils/resource.h"
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    psi::psi::PsiType psi_type;
    YACL_ENFORCE(psi::psi::PsiType_Parse(FLAGS_psi_protocol, &psi_type),
    ...
    psi::psi::BucketPsi ctx(config, link_ctx, FLAGS_ic_mode);
    auto report = ctx.Run();
    ...
    try {
      peak_mem_kb = psi::psi::GetPeakKbMemUsage();
