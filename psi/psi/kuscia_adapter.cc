...
KusciaTask FromKusciaConfig(const std::string& json_str) {
  ...
  kuscia_task.psi_config = task_input_config.sf_psi_config_map().at(self_party);
  ...
  kuscia_task.psi_config.mutable_link_config()->mutable_parties()->Clear();
  ...
  for (auto const& [id, host] : id_host_map) {
    auto* new_party =
        kuscia_task.psi_config.mutable_link_config()->add_parties();
  ...
  kuscia_task.psi_config.mutable_link_config()->set_brpc_channel_protocol(
      "http");
  kuscia_task.psi_config.set_self_link_party(self_party);
