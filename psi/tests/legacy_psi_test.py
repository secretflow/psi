...
from psi.tests.utils import get_free_port, wc_count, PropagatingThread
...
class UnitTests(unittest.TestCase):
    ...
    def test_ecdh_3pc(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            inputs = [
                "psi/tests/data/alice.csv",
                "psi/tests/data/bob.csv",
                "psi/tests/data/carol.csv",
            ]

            outputs = [
                os.path.join(tempdir, "alice-ecdh3pc.csv"),
                os.path.join(tempdir, "bob-ecdh3pc.csv"),
                os.path.join(tempdir, "carol-ecdh3pc.csv"),
            ]
    ...
    def test_kkrt_2pc(self):
        print("----------test_kkrt_2pc-------------")
        with tempfile.TemporaryDirectory() as tempdir:
            inputs = ["psi/tests/data/alice.csv", "psi/tests/data/bob.csv"]

            outputs = [
                os.path.join(tempdir, "alice-kkrt.csv"),
                os.path.join(tempdir, "bob-kkrt.csv"),
            ]
            selected_fields = ["id", "idx"]
    ...
    def test_ecdh_2pc(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            inputs = ["psi/tests/data/alice.csv", "psi/tests/data/bob.csv"]
            outputs = [
                os.path.join(tempdir, "alice-ecdh.csv"),
                os.path.join(tempdir, "bob-ecdh.csv"),
            ]
            selected_fields = ["id", "idx"]
    ...
    def test_dppsi_2pc(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            inputs = ["psi/tests/data/alice.csv", "psi/tests/data/bob.csv"]
            outputs = [
                os.path.join(tempdir, "alice-dppsi.csv"),
                os.path.join(tempdir, "bob-dppsi.csv"),
            ]
            selected_fields = ["id", "idx"]
    ...
    def test_ecdh_oprf_unbalanced(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            offline_path = ["", "psi/tests/data/bob.csv"]
            online_path = ["psi/tests/data/alice.csv", "psi/tests/data/bob.csv"]
            outputs = [
                os.path.join(tempdir, "alice-ecdh-unbalanced.csv"),
                os.path.join(tempdir, "bob-ecdh-unbalanced.csv"),
            ]
            preprocess_path = [os.path.join(tempdir, "alice-preprocess.csv"), ""]
            secret_key_path = ["", os.path.join(tempdir, "secret_key.bin")]
            selected_fields = ["id", "idx"]
            ...
            server_cache_path = os.path.join(tempdir, "server_cache.bin")
            ...
            def wrap(
                ...
                if receiver_rank != link_ctx.rank:
                    server_source_count = wc_count(offline_path)
                    self.assertEqual(
                        offline_report.original_count, server_source_count - 1
                    )
                ...
                if receiver_rank == link_ctx.rank:
                    ...
                    self.assertEqual(
                        report_online.original_count, client_source_count - 1
                    )

                print(f"online cost time: {time.time() - start}")
                print(
                    f"online: rank:{rank} original_count: {report_online.original_count}"
                )
