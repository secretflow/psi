...
3,"three","third"
    ...
    std::filesystem::path file_path = std::filesystem::temp_directory_path() /
                                      "check_csv_test_check_duplicates.csv";
    ...
    auto report = CheckCsv(file_path, {"id1"}, true, false);
    EXPECT_EQ(report.num_rows, 3);
    EXPECT_TRUE(report.contains_duplicates);
    EXPECT_TRUE(report.key_hash_digest.empty());

    std::error_code ec;
    std::filesystem::remove(report.duplicates_keys_file_path, ec);
...
3,"three","third"
    ...
    auto report = CheckCsv(file_path, {"id1"}, true, true);
    EXPECT_EQ(report.num_rows, 3);
    EXPECT_TRUE(report.contains_duplicates);
    EXPECT_FALSE(report.key_hash_digest.empty());
