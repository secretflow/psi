...
UbPsiCacheProvider::UbPsiCacheProvider(const std::string &file_path,
                                       size_t batch_size, size_t data_len)
    : batch_size_(batch_size), file_path_(file_path), data_len_(data_len) {
...
std::vector<std::string> UbPsiCacheProvider::ReadNextBatch() {
  ...
  size_t read_bytes = std::min<size_t>(batch_size_ * data_index_len_,
